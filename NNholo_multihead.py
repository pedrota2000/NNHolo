...
class CustomBundleSolver1D(BundleSolver1D):
    def __init__(self, sofT_list, phim_list, all_nets, V_net, g1, metric_flat_coef=100, UR=False, TL=False, *args, **kwargs):
     ...
     #       DDV = diff(diff(self.V_net[head](p), p), p)
      ...
      #      add_DV = ((DV - DV_th)**2).mean()
       #     add_DDV = ((DDV - DDV_th)**2).mean()
        ...
        if head == self.n_heads-1 and self.global_epoch% self.metric_flat_coef == 0 and self.TL == False and self.UR:
...
class NNholo_multihead_heist():
    ...
    def __init__(self, saving_path, sofT_path_list=None, phim_list=None, delta = 0.0, curriculum = 1.0, u_pts = 48, \
        ...
        load_path = None, UR = False, metric_flat_coef = 100, transfer_sofT_path = None, transfer_lr=1e-3, \
