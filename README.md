...
- Inside paper_2, there are two regimes: near-false-vacuum, and false vacuum.
    ...
    - For the near-false-vacuum regime ($\phi_M=0.8$), the code pipeline is defined in _NNholo_near_FV.py_ and executed in _near_FV_train_model.py_. The training is performed similarly to the paper I models. We also include a _run_and_analyze_pipeline.ipynb_ notebook to visualize the results.
      
    - For the false vacuum regime ($\phi_M = 0.55$), the training of the model is performed in two phases corresponding to the first branch (FB) and second branch (SB) of the equation of state curve $S(T)$. For the FB phase the pipeline is defined in _FB_main_pipeline.py_ and executed in _FB_training.ipynb_ for the FB phase. For the SB phase, the pipeline is defined in _SB_main_pipeline.py_ and executed in _SB_train_model.py_. For an analysis of the results after the SB training phase, we include also a _run_and_analyze_SB_pipeline.ipynb_ notebook.


For more details, see [paper I](https://arxiv.org/abs/2403.14763) and [paper II](https://arxiv.org/abs/2606.30117), respectively.

The pretrained models are available in the results directory (a light version with NN weights and the optimizer state), enough to load and visualize results. Full models with the loss function are also available upon request to the authors (contact details are available in the papers).
