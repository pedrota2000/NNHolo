...
class CustomBundleSolver1D(BundleSolver1D):
    def __init__(self, contrastive_weights=None, phim = None, u_pts = 48, mono_phi_coef=0, force_V_min_coef = 0, force_DV_DDV_min_coef = 0, force_V0_coef = 0,\
    ...
    def additional_loss(self,r,f,x):
        ...
        #     V_th_add_loss = derivs.mean()

        # Monotonic ordering of phi in the u direction #
        ...
        # add_loss = self.mono_coef* monotonic_addloss
        ...
        x = self.global_epoch
        x0 = torch.tensor([self.staging_addloss_epoch])   # Epoch at which loss starts gaining weight
        ...
        ###### Compute the full additional loss ###########
        ...
        add_loss =  force_V_min_coef * V_min_addloss + \
                    ...
                    # + self.mono_coef * monotonic_addloss + \
                    # add_loss_FB_val + \
                    # self.above_phi_c_coef * addloss_above_phi_c + \
...
class NNholo():
    ...
    def save_results(self, path):
        ...
        if 'add_loss' in losses.keys():  # Check if it exists for backward compatibility
            add_loss = losses['add_loss']
        else:
            add_loss = []

        if 'V_min_addloss' in losses.keys():  # Check if it exists for backward compatibility
            V_min_addloss = losses['V_min_addloss']
        else:
            V_min_addloss = []

        if 'DV_DDV_min_addloss' in losses.keys():  # Check if it exists for backward compatibility
            DV_DDV_min_addloss = losses['DV_DDV_min_addloss']
        else:
            DV_DDV_min_addloss = []

        if 'V0_addloss' in losses.keys():  # Check if it exists for backward compatibility
            V0_addloss = losses['V0_addloss']
        else:
            V0_addloss = []
        ...
        state = {'epoch': self.solver.global_epoch, 
                 ...
                 'train_loss': self.solver.metrics_history['train_loss'],
                 'add_loss': add_loss,
                 'V_min_addloss': V_min_addloss,
                 'DV_DDV_min_addloss': DV_DDV_min_addloss,
                 'V0_addloss': V0_addloss
    ...
    def load_results(self, path):
        ...
        if 'V_min_addloss' in master_dict:  # Check if it exists for backward compatibility
            self.solver.metrics_history['V_min_addloss'] = master_dict['V_min_addloss']

        if 'DV_DDV_min_addloss' in master_dict:  # Check if it exists for backward compatibility
            self.solver.metrics_history['DV_DDV_min_addloss'] = master_dict['DV_DDV_min_addloss']
        if 'V0_addloss' in master_dict:  # Check if it exists for backward compatibility
