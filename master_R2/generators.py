...
def _log_noise_inv(a,b,n,eps = 1.0):
    # Create log-spaced values in a transformed space
    log_values = np.logspace(0, eps, n) + np.random.rand(n)*0.1  # Values from 10^0 to 10^3

    # Normalize to range [0,1], reversing so that more points are near 1
    points, __ = torch.sort(torch.tensor(b - (b-a)*(log_values - min(log_values)) / (max(log_values) - min(log_values))),descending = True)
    points.requires_grad_(True)
    return points
...
class Generator1D(BaseGenerator):
    ...
    def __init__(self, size, t_min=0.0, t_max=1.0, method='uniform', noise_std=None,eps = 1.0):
        ...
        if method == 'uniform':
            ...
            self.getter = self.examples
        elif method == 'log-noisy-inv':
            self.examples = lambda: _log_noise_inv(t_min,t_max,size,eps)
            self.getter = self.examples
        else:
