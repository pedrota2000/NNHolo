...
class ext_V(nn.Module):
    def __init__(self, n_input_units, hidden_units, actv, n_output_units,n_NN):
        super(ext_V, self).__init__()
        self.VNN = [CustomNN(n_input_units = 1, hidden_units = [16,16,16,16] ,actv = nn.SiLU, n_output_units = 1) for _ in range(n_NN)]
        self.x_space = torch.linspace(0,2.0,len(self.VNN))
        
    def gaussian_filter(self,x):
        sigma = 2.0/len(self.VNN)
        #print(x_space)
        filter_list = []
        for i in range(len(self.x_space)):
            filter_list.append(torch.exp(-((x-self.x_space[i])**2)/(sigma)**2))
        return filter_list
    
    def forward(self,x):
        V_list = []
        gaussian = self.gaussian_filter(x)
        for i in range(len(self.VNN)):
            #plt.plot(x.detach().numpy(),gaussian[i].detach().numpy(),'.',label = str(i))
            V_list.append(self.VNN[i](x))# * gaussian[i])
        #V = torch.stack(V_list).squeeze().t()
        #print(V.shape)
        #plt.legend()
        #plt.show()
        V = sum(V_list)
        return V*(1-torch.exp(-x))**3 -3*(1+1/2* (1-torch.exp(-x))**2)
        #return V
...
class CustomBundleSolver1D(BundleSolver1D):
    ...
    def loss_fn(self,r,f,x):
        ...
        self.metrics_history['phi_max'].append(f[5][99].detach().item())
        return loss_r2
    ...
    def additional_loss(self,r,f,x):
        ...
        self.metrics_history['add_loss'].append((add_loss).detach().item())
...
class NNholo():
    ...
    def __init__(self, data_path, saving_path, init_pt_curve = 55, delta = 0.0, curriculum = 1.0,n_NN_V = 5):
        ...
        if suffix_data_path == ".txt":
            ...
            print('File is .txt')
            init_pt_curve = 5    #int(input('Init UV point S(T) ? (usually 0)'))
            S_true_1= torch.tensor(df_data[init_pt_curve:130:6,1])
            T_true_1= torch.tensor(df_data[init_pt_curve:130:6,0])
            #phi_h_true_1=torch.tensor(phih_true[50:100:10])

            #S_true_2= torch.tensor(df_data[71:100:5,1])
            #T_true_2= torch.tensor(df_data[71:100:5,0])

            S_true_3= torch.tensor(df_data[131:1250:15,1])
            T_true_3= torch.tensor(df_data[131:1250:15,0])
            #phi_h_true_2=torch.tensor(phih_true[101:200:5])

            S_true_4= torch.tensor(df_data[1251:-1:50,1])
            T_true_4= torch.tensor(df_data[1251:-1:50,0])

            #S_true_lowest = torch.tensor([1e-30])
            #T_true_lowest = torch.tensor([1e-30])
            
            S_true_1= torch.tensor(df_data[15:90:2,1])
            T_true_1= torch.tensor(df_data[15:90:2,0])
            #phi_h_true_1=torch.tensor(phih_true[50:100:10])
        ...
        # Rescale the inputs #
        #self.Sigma_uh_all = minmaxScaler(self.Sigma_uh_all/self.Va_uh_all).transform()
        #self.Va_uh_all = minmaxScaler(self.Va_uh_all).transform()
        ################################################################
        
        self.V = ext_V(n_input_units = 1, hidden_units = [32,32,32,32] ,actv = nn.SiLU, n_output_units = 1,n_NN = n_NN_V)
    ...
    BundleIVP(0, 0),  #phi(0)=0 #BundleDirichletBVP(0, 0,1, phi_yago[-1])#
        ...
        self.nets = [FCNN(n_input_units=3, hidden_units=[32,32,32],n_output_units = 1,
      ...
      #  self.adam = torch.optim.Adam(OrderedSet([p for net in self.nets + [self.V] for p in net.parameters()]), \
        ...
        param_nets = [p for net in self.nets for p in net.parameters()]
        param_V = [p for net in self.V.VNN for p in net.parameters()]
        param_V_total = [p for p in self.V.parameters()]
        print(param_V)
        #print(param_V_total)
        #param_diss_net = [p for p in self.nets[0].Diss_net.parameters()] 
        
        self.adam = torch.optim.Adam(param_nets+param_V, \
    ...
    def load_results(self, path):
        self.V.load_state_dict(torch.load(path)['state_dict_V'])
        self.adam.load_state_dict(torch.load(path)['optimizer'])
        self.solver = CustomBundleSolver1D( ode_system=self.equations,
                                            conditions=self.conditions,
                                            t_min=0.0,
                                            t_max=1.0,
        ...
        self.solver.metrics_history['r2_loss'] = torch.load(path)['loss']
        self.solver.metrics_history['train_loss'] = torch.load(path)['train_loss']
        self.solver.best_nets = np.ones_like(self.solver.nets)
        for i in range(len(self.solver.nets)):
            self.solver.nets[i].load_state_dict(torch.load(path)['state_dict_solver'][i])
            if torch.load(path)['state_best_nets'][i] != None:
                self.solver.best_nets[i] = self.solver.nets[i]
                self.solver.best_nets[i].load_state_dict(torch.load(path)['state_best_nets'][i])
