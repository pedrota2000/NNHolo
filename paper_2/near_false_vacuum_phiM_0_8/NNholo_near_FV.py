...
# Second derivative of the theoretical potential
# used by the additional V/DV/DDV-at-minimum losses (see CustomBundleSolver1D.additional_loss)
...
# Potential net with Gaussian localization
class CustomNN(nn.Module):
    def __init__(self, n_input_units, hidden_units, actv, n_output_units):
        ...
        # First hidden layer with Gaussian localization.
        # mu is learnable, sigma is now adapted to the layer width (2/hidden_units[0])
        # and the Gaussian uses the right denominator (2*sigma**2)
...
# Solver nets with Gaussian localization in the first layer applied along a chosen
# localization variable (u, S, T or affine index Z used when add_index=True
class CustomNets(nn.Module):
    def __init__(self, n_input_units=4, hidden_units=[64,64,64], actv=nn.Tanh, n_output_units=1, loc_var=3):
        ...
        # First hidden layer with fixed-mu/sigma Gaussian localization on the chosen localization variables
        self.layers.append(nn.Linear(n_input_units, hidden_units[0]))
        self.mu = torch.linspace(0.0, 1.0, self.hidden_units[0])
    ...
    def forward(self, x):
        # Localization variable index: u->0, S->1, T->2, Z->3
        inputx = x[:, self.loc_var].reshape(-1, 1)

        for i, layer in enumerate(self.layers):
            x = layer(x)
            if i == 0:
                # Localization is modulated by the learned weight column for the localization variable
...
# Callback: tracks the V network's localization parameters (mu, sigma, first-layer weights) and gradients
    ...
    def __call__(self, solver):
        ...
        for name, param in solver.V.named_parameters():
            if param.grad is not None:
                ...
                val=copy.deepcopy(param.grad.detach().abs().reshape(-1,1).cpu().detach().numpy())
...
class CustomBundleSolver1D(BundleSolver1D):
    def __init__(self, contrastive_weights=None, phim=None, u_pts=48,
                 mono_phi_coef=0, force_phi_H_mono_coef=0,
                 force_V_min_coef=0, force_DV_DDV_min_coef=0, force_V0_coef=0,
                 steep_step=100, staging_addloss_epoch=0, *args, **kwargs):

        self.V = kwargs.pop('V', None)

        # Coefficients for the additional physics-motivated losses
        self.mono_coef = mono_phi_coef
        self.steep_step = steep_step
        self.force_phi_H_mono_coef = force_phi_H_mono_coef
        self.force_V_min_coef = force_V_min_coef
        self.force_DV_DDV_min_coef = force_DV_DDV_min_coef
        self.force_V0_coef = force_V0_coef

        print('V_min_coef:', self.force_V_min_coef)
        print('DV_DDV_min_coef:', self.force_DV_DDV_min_coef)
        print('V0_coef:', self.force_V0_coef)

        self.phim = phim if phim is not None else 0.8

        # Pre-compute the theoretical V, DV, DDV at the V minimum once
        ...
        for k in ('r2_loss', 'phi_max', 'add_loss', 'monotonic_phi_add_loss',
                  'V_min_addloss', 'DV_DDV_min_addloss', 'V0_addloss', 'phi_H_mono_addloss'):
            self.metrics_history[k] = []

        self.u_pts = u_pts
        u = self.generator['train'].get_examples()[0]
    ...
    def loss_fn(self, r, f, x):
        # Contrastive re-weighting per-(S,T)-curve scaling of the residuals (NOT used for the final phiM = 0.8 run)
        if self.contrastive_weights is not None:
            w0 = torch.tensor(self.contrastive_weights).T          # shape (sofT_pts, 7)
            w = w0.repeat(self.u_pts, 1)                          
            loss_r2 = (w * r**2).mean()
        else:
            loss_r2 = (r**2).mean()
    ...
    # Additional losses:
    # Four physically-motivated terms, each turned on via its own `force_*_coef`term and
    # gradually staged in via a sigmoid during training:
    #   * V0           : boundary values  V(0) = -3, V'(0) = 0, V''(0) = -3
    #   * V_min        : NN potential at phi_H matches the theoretical V minimum
    #   * DV_DDV_min   : 1st & 2nd derivatives at phi_H match the theoretical ones
    #   * phi_H_mono   : phi(u=1) is monotonic across the (S,T) bundle (NOT used for the final phiM = 0.8 run)
    def additional_loss(self, r, f, x):

        # V(0), V'(0), V''(0) boundary loss
        if self.force_V0_coef != 0:
            phi0 = f[5][0].reshape(-1, 1) * 0.0
            V = self.V(phi0)
            DV = diff(self.V(phi0), phi0, shape_check=False)
            DDV = diff(diff(self.V(phi0), phi0, shape_check=False), phi0, shape_check=False)
            V0_addloss = (3 + DDV[-1])**2 + (0 - DV[-1])**2 + (3 + V[-1])**2
        else:
            V0_addloss = torch.tensor([0.0])

        # V(phi_H) matches theoretical V minimum
        if self.force_V_min_coef != 0:
            p = f[5].reshape(self.u_pts, self.sofT_pts)
            p_last = p[0, -1].reshape(-1, 1)             # phi at u=1 for last (S,T) point

            Vth_min = torch.tensor(self.Vth_min)
            Vnn_min = self.V(p_last)
            if self.global_epoch == 0:
                print('Vmin (u=1, (S,T)=(0,0)) = ', Vnn_min.item(),
                      ' at phi = ', p_last.item(), ' with Vth_min = ', Vth_min.item())
                print('--------------------------------------------------------------------------')
        ...
        # V'(phi_H), V''(phi_H) match theoretical values at the minimum
        if self.force_DV_DDV_min_coef != 0:
            p = f[5].reshape(self.u_pts, self.sofT_pts)
            p_last = p[0, -1].reshape(-1, 1)
            ...
            DV_DDV_min_addloss = (DVmin_nn - DVmin_th)**2 + (DDVmin_nn - DDVmin_th)**2
        else:
            DV_DDV_min_addloss = torch.tensor([0.0])

        # phi_H monotonic across the (S,T) bundle
        if self.force_phi_H_mono_coef != 0:
            p = f[5].reshape(self.u_pts, self.sofT_pts)
            phi_H = p[0, :]
            diffs = phi_H[1:] - phi_H[:-1]
            phi_H_mono_addloss = torch.relu(-diffs).mean()
        else:
            phi_H_mono_addloss = torch.tensor([0.0])

        # Sigmoid staging of the loss coefficients in global epoch
        ep = self.global_epoch
        ep0 = torch.tensor([self.staging_addloss_epoch])  # turn-on epoch
        ep_off = torch.tensor([200000.0])                 # turn-off epoch (only for the mono term)
        a = 1e-4                                          # staging slope

        y = torch.sigmoid(a * (ep - ep0))

        force_V_min_coef       = self.force_V_min_coef       * y
        force_DV_DDV_min_coef  = self.force_DV_DDV_min_coef  * y
        force_V0_coef          = self.force_V0_coef          * y
        force_phi_H_mono_coef  = self.force_phi_H_mono_coef  * y * (1 - torch.sigmoid(a * (ep - ep_off)))

        if self.global_epoch == 0:
            print('Vmin coef (staged):', y, force_V_min_coef)
            print('DV_DDV_min coef (staged):', y, force_DV_DDV_min_coef)
            print('V0 coef (staged):', y, force_V0_coef)
            print('phi_H mono coef (staged):', y, force_phi_H_mono_coef)

        add_loss = (force_V_min_coef      * V_min_addloss
                    + force_DV_DDV_min_coef * DV_DDV_min_addloss
                    + force_V0_coef         * V0_addloss
                    + force_phi_H_mono_coef * phi_H_mono_addloss)

        self.metrics_history['V_min_addloss'].append((force_V_min_coef * V_min_addloss).cpu().detach().item())
        self.metrics_history['DV_DDV_min_addloss'].append((force_DV_DDV_min_coef * DV_DDV_min_addloss).cpu().detach().item())
        self.metrics_history['V0_addloss'].append((force_V0_coef * V0_addloss).cpu().detach().item())
        self.metrics_history['phi_H_mono_addloss'].append((force_phi_H_mono_coef * phi_H_mono_addloss).cpu().detach().item())
...
# IMPORT DATA (reference solutions for comparison)
...
# 3of5 reference (S*, T*, phi_h*) triplets
...
class NNholo():
    ...
    def __init__(self, data_path, saving_path, contrastive_weights=None, n_points = 69, T_min=0.001, u_pts = 48,\
        ...
        # Affine-parameter uniform sampling:

        # Re-sample the (S,T) curve uniformly in arc length so that bundle points are
        # equispaced along the curve. The normalized arc length Z is later passed as
        # an extra bundle parameter to the solver nets
        Z_list = torch.arange(0, len(self.Sigma_uh_all.cpu().detach().numpy()), 1)
    ...
    BundleIVP(0, 0),  #phi(0)=0 #BundleDirichletBVP(0, 0,1, phi_yago[-1])#
        ...
        # Solver nets take 4 inputs (u, S, T, Z) when add_index=True; Z is the affine-parameter index
        if add_index:
        ...
        self.solver = CustomBundleSolver1D( ode_system=self.equations,
                                            ...
                                            force_DV_DDV_min_coef = self.force_DV_DDV_min_coef,
                                            force_V0_coef = self.force_V0_coef,
                                            staging_addloss_epoch = self.staging_addloss_epoch,
                                            force_phi_H_mono_coef = self.force_phi_H_mono_coef,
                                        )
    def sofT_curve(self):
    ...
    def equations(self, Vs, Va, Vp, Sigma, A, phi, u):
        ...
        # eq6 rescaled by 1/max(Vp, 1e-3) for numerical stability when Vp -> 0
    ...
    def plot_additional_losses(self, save_fig = True):
        ...
        if 'phi_H_mono_addloss' in loss and len(loss['phi_H_mono_addloss']) > 0:
            plt.plot(loss['phi_H_mono_addloss'][::step], label = r'$\phi_H$ mono')
        plt.legend()
        plt.yscale('log')
        plt.xlabel('Epochs')
        plt.ylabel('Loss')
    ...
    def plot_separate_losses_new(self, past_epochs = None, save_fig = True):
        ...
        DE_loss = np.array(loss['r2_loss'])
        if 'above_phi_c_addloss' in loss.keys():
            above_phi_c_addloss = np.array(loss['above_phi_c_addloss'])
        if 'phiH_ordering_addloss' in loss.keys():
            pH_ordering_addloss = np.array(loss['phiH_ordering_addloss'])
        ...
        # for i in range(len(DVDDV_min_addloss)):
        #     if DVDDV_min_addloss[i]>1e-8:
        #         # print('first', i)
        #         threshold_epoch = i
        #         break

        # print(self.solver.global_epoch - len(DVDDV_min_addloss))
        # if self.solver.global_epoch - len(DVDDV_min_addloss) > 0:
        #     DVDDV_min_addloss = np.concatenate([np.zeros(self.solver.global_epoch - len(DVDDV_min_addloss)), DVDDV_min_addloss])
        ...
        min_tot_loss = []
        min_DE_loss = []
        min_above_phi_c_addloss = []
        min_pH_ordering_addloss = []
        min_Vmin_addloss = []
        min_DVDDV_min_addloss = []
        min_V0_addloss = []
        ...
        for i in range(len(tot_loss)):
            ...
            if i == 0:
                ...
                # DV/DDV min addloss: only starts tracking after threshold_epoch
        ...
        # if self.solver.order_phi_H_coef != 0:
        #     plt.plot(min_pH_ordering_addloss[-past_epochs:], label=r'$\phi_H$-ordering addloss', alpha=1)
        # if self.solver.above_phi_c_coef != 0:
        #     plt.plot(min_above_phi_c_addloss[-past_epochs:], label=r'$\phi_c$-above addloss', alpha=1)
        if self.solver.force_V_min_coef != 0:
        ...
        if self.solver.force_V0_coef != 0:
    ...
    # Saves the V network, solver nets, optimizer state and all loss histories
    # (including the additional losses). phi_c is the last phi at u=1.
    def save_results(self, path):

        nets_state = [net.state_dict() for net in self.solver.nets]
        best_nets_state = [net.state_dict() for net in self.solver.best_nets]
        ...
        for i in range(len(self.Va_uh_all)):
            ...
            Z = self.Z[i].cpu().detach().numpy() * np.ones_like(u.cpu().detach().numpy())
            sol = self.solver.get_solution(best=True)
            phi_solutions.append(sol(u, S, T, Z)[5].cpu().detach().numpy())

        phi_c = phi_solutions[-1][-1]

        losses = self.solver.metrics_history
        state = {'epoch': self.solver.global_epoch,
                 ...
                 'add_loss': losses.get('add_loss', []),
                 'V_min_addloss': losses.get('V_min_addloss', []),
                 'DV_DDV_min_addloss': losses.get('DV_DDV_min_addloss', []),
                 'V0_addloss': losses.get('V0_addloss', []),
                 'phi_H_mono_addloss': losses.get('phi_H_mono_addloss', [])}

        torch.save(state, path + '_epochs_' + str(len(losses['r2_loss'])))
    ...
    def load_results(self, path):
        ...
        # Only restore additional-loss histories if present in the file.
        for key in ('add_loss', 'V_min_addloss', 'DV_DDV_min_addloss',
                    'V0_addloss', 'phi_H_mono_addloss'):
            if key in master_dict:
                self.solver.metrics_history[key] = master_dict[key]
        ...
        for i in range(len(self.solver.nets)):
            ...
            if master_dict['state_best_nets'][i] is not None:
